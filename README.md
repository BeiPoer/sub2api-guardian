...
| 调度倍率 | 每个渠道一个倍率，越低越优先。渠道管理按 URL + 完整 API Key 匹配成功时同步用户分组倍率；未联动的 API Key 可按运营配置的周期（最低 30 秒）读取上游倍率或立即同步，失败保留最近成功值。**仅供调度系统使用，不写回 sub2api** |
...
Sub2API 或 New API 分组与令牌同步成功后，会用规范化的上游 URL + 完整 API Key 精确匹配渠道池账号，
按 `令牌分组倍率 ÷ 充值比例` 换算并同步 Guardian 调度倍率，再将账号名称幂等更新为 `原名称【x倍率】`；
例如渠道充值比例为 `1:10`、令牌倍率为 `0.15` 时，账号调度倍率为 `0.015`。匹配只处理完整凭据，
URL 比较仅忽略协议和主机大小写、去除末尾 `/` 并保留路径，Key 比较大小写敏感；
New API 列表接口返回脱敏 Key 时，渠道管理同步会按令牌 ID 读取完整 Key 并写入令牌缓存；
读取失败时仍按脱敏 Key 跳过。本次联动读取的渠道池导出凭据不会落库、返回前端或写入日志；URL/Key 失配、
