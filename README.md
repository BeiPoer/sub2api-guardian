...
| 调度倍率 | 每个渠道一个倍率，越低越优先。渠道管理按完整 API Key 匹配成功时同步用户分组倍率；未联动的 API Key 可按运营配置的周期（最低 30 秒）读取上游倍率或立即同步，失败保留最近成功值。**仅供调度系统使用，不写回 sub2api** |
...
| 上游渠道 | 独立管理 Sub2API、New API 与其它站点，查看余额、分组、令牌、订阅并配置邮件/企微告警 |
...
侧栏「渠道管理」（与「系统」同级）包含「渠道汇总」和「渠道列表」：汇总页集中展示余额、
令牌与倍率，列表页在左侧选择渠道、右侧查看详情。该模块使用独立的数据与接口；Sub2API
Sub2API 或 New API 分组与令牌同步成功后，会用完整 API Key 精确匹配渠道池账号，
按 `令牌分组倍率 ÷ 充值比例` 换算并同步 Guardian 调度倍率，再将账号名称幂等更新为 `原名称【x倍率】`；
例如渠道充值比例为 `1:10`、令牌倍率为 `0.15` 时，账号调度倍率为 `0.015`。匹配只处理完整凭据，
匹配不比较上游 URL；Key 比较大小写敏感，New API 额外兼容令牌 Key 是否带 `sk-` 前缀；
New API 列表接口返回脱敏 Key 时，渠道管理同步会按令牌 ID 读取完整 Key 并写入令牌缓存；
读取失败时仍按脱敏 Key 跳过。本次联动读取的渠道池导出凭据不会落库、返回前端或写入日志；Key 失配、
无效倍率和重复 Key 的冲突候选均跳过且保留旧值。
手工编辑渠道池调度倍率会解除该账号的本次联动。支持 Sub2API / New API 同步、余额历史、分组与令牌、
订阅、令牌模型和分组修改，以及低余额、消耗速率、分组新增/移除/倍率变化五类邮件与企微应用告警；
上述倍率联动同时支持 Sub2API 和 New API；两者都要求令牌 Key 完整且能解析出有效分组倍率。
...
G2 仍需保留自己的 Sub2API 账号目录，G1/G2 的账号名称和上游 URL 可以不同；同步按完整 API Key 匹配，匹配成功后复用本地倍率联动逻辑，设置渠道池调度倍率并更新 G2 账号名称后缀（例如 `渠道【x0.015】`）。G1 快照暂时不完整或授权失效时，G2 对无法确认的账号保留最近一次成功倍率，不会用不完整结果整体清空现有配置。生产环境建议使用 HTTPS 访问 G1。

G1/G2 使用共享倍率源时，两端需要运行同一协议版本；纯 API Key 匹配使用倍率源协议 v2。
...
余额快照、余额查询日志和告警事件保留 7 天；渠道配置、缓存、任务与邮件/企微设置不会随历史清理删除。
...
- **上游渠道凭据可逆保存并直接展示**：上游密码和 New API 系统令牌需要用于远端登录，
  ...
  access / refresh token 不通过 API 返回，SMTP 密码和企微应用 Secret 也只显示“已配置”状态
- **改密码会吊销其他会话**，只保留当前这一个
...
backend/
  ...
  internal/
    ...
    channelmanager/      独立上游渠道同步、余额、令牌、告警、SMTP 与企微应用
